...
* PostgreSQL Host: `postgres.k8s.tss.local`, Port: `5432`, Database: `gitlabhq_production`, User: `gitlab`
...
- [ ] **PostgreSQL**: Reachable at `postgres.k8s.tss.local:5432`, `gitlabhq_production` DB created, user `gitlab` granted privileges and required extensions (`pg_trgm`, `btree_gist`, `amcheck`).
